...
export const command = new SlashCommandBuilder()
  .setName("leaderboard")
  .setDescription("View leaderboard by Jades or Credits")
  .addStringOption(option =>
    option
      .setName("type")
      .setDescription("Credits or Jades")
      .setRequired(true)
      .addChoices(
...
export async function execute(interaction) {
    ...
    try {
        ...
        const memberIds = new Set(
            fetchedMembers.filter(m => m).map(m => m.user.id)
        );
        entries = entries.filter(([id]) => memberIds.has(id));
       
        }
        entries.sort((a, b) => (b[1][type] || 0) - (a[1][type] || 0));
        ...
        .map(([id, data], i) => {
            ...
            return `**#${i + 1}** <@${id}> — ${value.toLocaleString()} ${type}`;
        })
        .join("\n");
        const embed = new EmbedBuilder()
        .setTitle(`${type} Leaderboard (${scope === "here" ? "Server" : "Global"})`)
